...
Component.entryPoint = function(){
	...
	ManagerWidget.prototype = {
		...
		dsComplete: function(type, args){
			if (args[0].checkWithParam('payment', {})){
				if (type == 'onComplete'){
					this.render(); 
				}else{
					this.renderWait();
		...
		render: function(){
			...
			DATA.get('payment').getRows().foreach(function(row){
				var di = row.cell;
				lst += TM.replace('row', {
					'tl': di['tl'],
					'ord': di['ord'],
					'id': di['id'],
					'checked': di['def']*1>0 ? 'checked="checked"' : ''
				});
			});
		...
		save: function(){
			...
			table.getRows().foreach(function(row){
				row.update({
					'ord': Dom.get(TM.getElId('row.ord')+'-'+row.id).value,
					'def': Dom.get(TM.getElId('row.def')+'-'+row.id).checked ? "1" : "0"
				});
			});
			table.applyChanges();
			DATA.request();
	...
	NS.payment.ManagerWidget = ManagerWidget;
	
	API.showPaymentManagerWidget = function(container){
	...
	NS.payment.ManagerPanel = ManagerPanel;
	
	API.showPaymentManagerPanel = function(){
